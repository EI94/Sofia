...
def verify_api_key(
    ...
    if not expected_key:
        raise HTTPException(status_code=500, detail="BULK_API_KEY non configurato")
...
async def call_core_sofia(messages: List[Dict[str, str]]) -> str:
    ...
    if not core_url:
        raise HTTPException(status_code=500, detail="CORE_SOFIA_URL non configurato")
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                data = response.json()
                return data.get(
                    "reply", "Mi dispiace, non ho potuto generare una risposta."
                )
            else:
...
async def process_conversation(
    ...
    # Salva nel database
    upsert_conv(conversation.conversation_id, output.model_dump())
...
async def get_conversation(
    request: Request, cid: str, api_key: str = Depends(verify_api_key)
) -> ConversationOut:
    """
...
async def root():
    """Root endpoint"""
    return {"message": "Sofia Bulk API", "version": "1.0.0", "docs": "/docs"}
