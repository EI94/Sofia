...
def test_executor_guardrails_integration():
    ...
    assert (
        ctx.slots.get("abuse_count") == 1
    ), f"Expected abuse_count=1, got: {ctx.slots.get('abuse_count')}"
...
def test_edge_cases():
    ...
    assert not is_inappropriate(
        special_text
    ), "Text with special chars should not be inappropriate"
