...
class TestOrchestrator:
    ...
    def setup_method(self):
        ...
        self.test_ctx = Context(
            phone=self.test_phone, lang="it", state="GREETING", name="Test User"
        )
    ...
    def test_process_message_existing_user(self):
        ...
        with (
            patch(
                "sofia_lite.middleware.memory.load_context", return_value=self.test_ctx
            ),
            patch("sofia_lite.middleware.memory.save_context") as mock_save,
            patch(
                "sofia_lite.agents.prompt_builder.build_system_prompt",
            ...
            patch("sofia_lite.agents.validator.validate", return_value=True),
            patch(
                "sofia_lite.agents.executor.dispatch", return_value="Come ti chiami?"
            ),
        ):
    ...
    def test_process_message_invalid_intent(self):
        ...
        with (
            ...
            patch(
                "sofia_lite.agents.executor.dispatch",
                return_value="Mi dispiace, puoi ripetere?",
            ),
        ):

            result = self.orchestrator.process_message(
                self.test_phone, "messaggio strano"
            )
