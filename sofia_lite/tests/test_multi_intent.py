...
from sofia_lite.agents.planner import classify_intents, plan, next_state
...
def test_classify_intents_single():
    ...
    for text, expected in test_cases:
        ...
        assert expected[0] in result, f"Failed for '{text}': expected {expected[0]}, got {result}"
...
def test_classify_intents_multi():
    ...
    test_cases = [
        ...
        ("Salve quanto costa la cittadinanza?", ["REQUEST_SERVICE", "ASK_COST", "GREET"]),
        ("Mi chiamo Maria e vorrei sapere i costi", ["ASK_NAME", "ASK_COST", "REQUEST_SERVICE"]),
    ...
    for text, expected in test_cases:
        ...
        assert result[0] == expected[0], f"Failed for '{text}': expected first intent {expected[0]}, got {result[0]}"
        
        # Verifica che tutti gli intent attesi siano presenti
        for intent in expected:
            assert intent in result, f"Failed for '{text}': expected {intent} in {result}"
...
def test_priority_order():
    ...
    assert result[0] == "REQUEST_SERVICE", f"REQUEST_SERVICE should be first, got {result[0]}"
...
def test_next_state_mapping():
    ...
    for current_state, intent, expected_state in test_cases:
        ...
        assert result == expected_state, f"Failed for {current_state} + {intent}: expected {expected_state}, got {result}"
...
def test_plan_with_multi_intent():
    ...
    assert "Multi-intent detected" in reason, f"Expected multi-intent reason, got {reason}"
...
def test_edge_cases():
    ...
    assert "REQUEST_SERVICE" in result, f"Long text should still detect REQUEST_SERVICE, got {result}"
