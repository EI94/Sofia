...
class LoopGuard:
    ...
    def check_loop(self, ctx: Context, intent: str, response: str) -> Dict[str, Any]:
        ...
        if self.clarification_count >= self.max_clarifications:
            log.warning(
                f"🔄 Clarification loop detected for {ctx.phone}, escalating to human"
            )
            return {
                "escalate": True,
        ...
        if len(ctx.history) > 0:
            recent_responses = [
                msg["content"] for msg in ctx.history[-3:] if msg["role"] == "assistant"
            ]
            if (
                len(recent_responses) >= 2
                and recent_responses[-1] == recent_responses[-2]
            ):
                log.warning(
                    f"🔄 Repeated response detected for {ctx.phone}, escalating to human"
                )
                return {
                    "escalate": True,
        ...
        if len(ctx.history) > self.escalation_threshold * 2:
            log.info(
                f"📊 Long conversation detected for {ctx.phone}, considering escalation"
            )
            return {
                "escalate": True,
        ...
        return {"escalate": False, "reason": None, "message": None}
    ...
    def get_escalation_message(self, ctx: Context, intent: str, response: str) -> str:
        ...
        result = self.check_loop(ctx, intent, response)
        return result.get(
            "message", "Mi dispiace, ti metto in contatto con un consulente umano."
        )
