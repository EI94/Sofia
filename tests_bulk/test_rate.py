...
class TestRateLimiting:
    ...
    def test_rate_limit_10_requests_per_second(self):
        ...
        for response in responses:
            assert response.status_code in [200, 500]  # 500 se CORE_SOFIA_URL non raggiungibile
    ...
    def test_rate_limit_per_api_key(self):
        ...
        for i in range(10):
            response = self.client.post(self.base_url, json=conversation_data, headers=headers1)
        ...
        response = self.client.post(self.base_url, json=conversation_data, headers=headers2)
        assert response.status_code in [200, 500]
