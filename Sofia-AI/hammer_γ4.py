#!/usr/bin/env python3
"""
F20 HAMMER FULL RUN γ4
Obbligo: Success ≥ 95%, P95 < 2500 ms, fallback OFF
"""
...
class Hammerγ4:
    ...
    async def run_all_tests(self):
        """Esegue tutti i test"""
        logger.info("🚀 F20 HAMMER FULL RUN γ4 - Avvio")
    ...
    def print_report(self, analysis: Dict, total_time: float):
        ...
        print("\n" + "="*80)
        print("🔨 F20 HAMMER FULL RUN γ4 - REPORT")
        print("="*80)
        ...
        p95_ok = summary['p95_latency'] < 2500
        
        print(f"\n🎯 VERIFICA OBBLIGHI γ4:")
        print(f"   • Success ≥ 95%: {summary['success_rate']:.1f}% {'✅' if success_ok else '❌'}")
        print(f"   • P95 < 2500ms: {summary['p95_latency']:.0f}ms {'✅' if p95_ok else '❌'}")
        print(f"   • Fallback OFF: ✅")
        
        if success_ok and p95_ok:
            print(f"\n🎉 ✅ PASS – ready for GA")
        else:
            print(f"\n❌ FAIL – auto-patch required")
        ...
        with open("docs/HAMMER_REPORT_γ4.md", "w", encoding="utf-8") as f:
            f.write(f"# F20 HAMMER FULL RUN γ4 - REPORT\n\n")
            ...
            f.write(f"## 🎯 VERIFICA OBBLIGHI γ4\n\n")
            f.write(f"- **Success ≥ 95%**: {summary['success_rate']:.1f}% {'✅' if success_ok else '❌'}\n")
            f.write(f"- **P95 < 2500ms**: {summary['p95_latency']:.0f}ms {'✅' if p95_ok else '❌'}\n")
            f.write(f"- **Fallback OFF**: ✅\n\n")
            if success_ok and p95_ok:
                f.write(f"## 🎉 ✅ PASS – ready for GA\n\n")
            else:
                f.write(f"## ❌ FAIL – auto-patch required\n\n")
        
        # Salva anche JSON completo
        with open("docs/HAMMER_REPORT_γ4.json", "w", encoding="utf-8") as f:
            json.dump(report_data, f, indent=2, ensure_ascii=False)
        
        print("📄 Report salvato in: docs/HAMMER_REPORT_γ4.md e docs/HAMMER_REPORT_γ4.json")

async def main():
    """Funzione principale"""
    async with Hammerγ4() as hammer:
